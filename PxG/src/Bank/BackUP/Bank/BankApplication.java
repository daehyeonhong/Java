package Bank.BackUP.Bank;

import java.util.Scanner;

public class BankApplication {
	private static Account[] accountArray = new Account[2];
	...
	private static void createAccount() {
		int fir = (int) (Math.random() * 1000);
		int seco = (int) (Math.random() * 100);
		int last = (int) (Math.random() * 10000);
		String ano = (fir + "-" + seco + "-" + last);
		if (checkAno(ano)) {
			System.out.println("--------");
				...
				for (int i = (int) (Math.random() * accountArray.length); i < accountArray.length;) {
					if (accountArray[i] == null) {
						...
						if (balance < 0) {
							...
							System.out.println(
									accountArray[i].getOwner() + "님 계좌가 생성되었습니다.\n계좌번호는 " + accountArray[i].getAno());
							break;
						}
					}
				}
	...
	private static boolean checkAno(String ano) {
		for (int i = 0; i < accountArray.length; i++) {
			if (!(accountArray[i] == null)) {
