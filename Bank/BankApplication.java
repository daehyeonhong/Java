...
public class BankApplication {
		...
		if (checkArray() < accountArray.length) {
			...
			if (setPassword(password) == null) {
				...
				if (balance < 0) {
					...
					int check = checkArray();
					accountArray[check] = new Account(makeAno(), owner, balance, password);
					System.out.println(accountArray[check].getOwner() + "님 계좌가 생성되었습니다.\n계좌번호는 "
							+ accountArray[check].getAno() + "입니다.");
				}
			}
		} else {
	...
	private static String makeAno() {
		...
		while (true) {
			...
			while (fir > (f / 10) && middle > (m / 10) && last > (l / 10)) {
				ano = (fir + "-" + middle + "-" + last);
				break;
			}
