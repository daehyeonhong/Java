...
public class asd {
			...
			if (balance >= 0) {
				...
				for (int i = 0; i < accountArray.length; i++) {
					if (accountArray[i] == null) {
						...
						System.out.println("결과:" + account.getOwner() + "님 계좌가 생성되었습니다.\n계좌번호:" + account.getAno());
						break;
					}
				}
			} else {
		...
		if (account != null) {
			if (account.isLogon() == true) {
				...
				if (balance > 0) {
					...
					System.out.println("예금액을 확인 후 다시 시도하세요.");
				}
			} else {
	...
	private static void changePassword() {
		...
		if (account != null) {
			if (account.isLogon() == true) {
				System.out.println(account.getOwner() + "님 환영합니다. 비밀번호 변경을 위해 비밀번호를 입력해주세요.");
				// 입력된 값들이 서로 같은 객체를 참조하고 있는지를 확인합니다.
				...
				if (pwd3Check(ano) != null) {
					...
					if (againPassword.equals(newPassword)) {
						account.setPassword(againPassword);
						System.out.println("비밀번호 변경이 완료 되었습니다.");
					} else
						System.out.println("비밀번호 확인 오류!!! 다시 시도하세요.");
				}
			} else {
				System.out.println("로그인 후 다시 시도하세요.");
			}
	...
	private static void secession() {
		...
		if (account != null) {
			if (pwd3Check(ano) != null) {
				System.out.print(account.getOwner() + "님" + account.getAno() + "계좌를 정말 탈퇴하시겠습니까?(y/n)");
				char yesNo = scanner.next().charAt(0);
	...
	private static Account findAccount(String ano) {// 계좌번호를 받고 계좌를(Account객체)리턴
		...
		if (findAccount(ano) != null) {
			for (int i = 0; i < 3; i++) {
				...
				if (run == true) {
					...
					if (i < 2) {
						System.out.println(((3 - 1) - (i)) + "회 남았습니다.");
					} else if (i == 2) {
